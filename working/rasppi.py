...
SAMPLE_RATE = int(os.getenv('SAMPLE_RATE', 20))
WINDOW_TIME = int(os.getenv('WINDOW_TIME', 1))
WINDOW_SIZE = SAMPLE_RATE * WINDOW_TIME
REFERENCE_VOLTAGE = float(os.getenv('REFERENCE_VOLTAGE', 0.0006))
SPIKE_THRESHOLD_DB = int(os.getenv('SPIKE_THRESHOLD_DB', 8))
LOUD_THRESHOLD_DB = int(os.getenv('LOUD_THRESHOLD_DB', 65))

# Gas Sensor Parameters
RLOAD = int(os.getenv('RLOAD', 10000))
VCC = 5.0
R0 = int(os.getenv('R0', 5000))
MQ135_CLEAN_AIR_RATIO = float(os.getenv('MQ135_CLEAN_AIR_RATIO', 3.6))

# Odor Classification Thresholds
VOC_CLEAN_THRESHOLD = int(os.getenv('VOC_CLEAN_THRESHOLD', 20))
VOC_ACTIVITY_THRESHOLD = int(os.getenv('VOC_ACTIVITY_THRESHOLD', 40))
VOC_CHEMICAL_THRESHOLD = int(os.getenv('VOC_CHEMICAL_THRESHOLD', 60))
...
MMWAVE_TX_PIN = 24  # GPIO pin for mmWave TX
MMWAVE_RX_PIN = 23  # GPIO pin for mmWave RX
...
class SoftwareUART:
    ...
    def __init__(self, tx_pin, rx_pin, baudrate=256000):
        ...
        self.pi.set_pull_up_down(self.rx_pin, pigpio.PUD_UP)
    ...
    def _connect_and_detect(self):
        if not self.use_software_uart:
            try:
                ...
                self.detected_radar_type = 'ld2450'
                self.config = RADAR_CONFIGS['ld2450']
                logger.info(f"✅ Hardware UART connected on /dev/ttyAMA0 at 256000 baud")
            except Exception as e:
                logger.error(f"❌ Failed to connect hardware UART: {e}")
            ...
            while True:
                ...
                if byte[0] == 0xAA:
                    ...
                    if next_byte and next_byte[0] == 0xFF:
                        rest = self.serial_conn.read(28)
                        raw_data = byte + next_byte + rest
                        break
            if raw_data:
...
class RadarProcessor:
    ...
    def _connect_and_detect(self):
            if not self.use_software_uart:
                try:
                    ...
                    self.detected_radar_type = 'ld2450'
                    self.config = RADAR_CONFIGS['ld2450']
                    logger.info("✅ Hardware UART connected on /dev/ttyAMA0 at 256000 baud")
                except Exception as e:
                    logger.error(f"❌ Failed to connect hardware UART: {e}")
                    self.serial_conn = None
                    self.detected_radar_type = 'ld2450'
                ...
                try:
                    ...
                    self.detected_radar_type = 'ld2450'
                    self.config = RADAR_CONFIGS['ld2450']
                    logger.info(f"✅ Software UART for LD2450 on TX:{MMWAVE_TX_PIN}, RX:{MMWAVE_RX_PIN}")
                except Exception as e:
    ...
    def _parse_ld2450_frame(self, data):
        """Parse HLK-LD2450 radar frame format"""
        # Find frame header AA FF 03 00
        ...
        targets = []
        # Each target is 8 bytes, starting after 4-byte header
        for i in range(3):
            offset = 4 + i * 8
            if offset + 8 > len(frame) - 2:
                break
            # X coordinate (signed, little endian)
            x_raw = (frame[offset+1] << 8) | frame[offset]
            if x_raw & 0x8000:
                x = -(x_raw & 0x7FFF)
            else:
                x = x_raw & 0x7FFF
            x = x / 1000.0  # convert mm to meters
        
            # Y coordinate (signed, little endian)
            y_raw = (frame[offset+3] << 8) | frame[offset+2]
            if y_raw & 0x8000:
                y = -(y_raw & 0x7FFF)
            else:
                y = y_raw & 0x7FFF
            y = y / 1000.0  # convert mm to meters
        
            # Speed (signed, little endian)
            speed_raw = (frame[offset+5] << 8) | frame[offset+4]
            if speed_raw & 0x8000:
            ...
            orientation = 'toward' if speed < 0 else 'away' if speed > 0 else 'stationary'
            ...
            target = {
                ...
                'angle': round(angle, 1),
                'velocity': round(abs(speed), 3),
                'direction': 'incoming' if speed < 0 else 'outgoing',
    ...
    def parse_radar_frame(self, raw_data):
        """Parse raw radar data"""
        if not raw_data:
            return None
        try:
            return self._parse_ld2450_frame(raw_data)
        except Exception as e:
...
class TemporalThreatScorer:
    ...
    def exponential_escalation(self, base_threat: float, time_active: float, 
        ...
        time_active = min(time_active, 300)  # Cap at 5 minutes
...
class EnhancedThreatScorer:
    ...
    def calculate_count_threat(self, target_count: int) -> Tuple[float, float]:
        """Calculate threat based on number of people detected"""
        if target_count == 0:
            return 0, 0.9  # No threat, high confidence
    
        # Exponential threat increase with count
        if target_count >= 5:
            threat_score = 60 + (target_count - 5) * 10  # 60+ for 5+ people
        elif target_count == 4:
            threat_score = 45
        elif target_count == 3:
            threat_score = 30
        ...
        # Confidence decreases with higher counts (less certainty)
        confidence = max(0.5, 0.95 - (target_count * 0.05))
    ...
    def calculate_overall_threat(self, radar_data: Optional[Dict], 
        ...
        behavior_score_raw = self.calculate_behavior_threat(targets, motion_patterns, activity_events)
        proximity_score_raw = behavior_score_raw  # Same calculation as behavior
        
        components_raw = {
            'count': self.calculate_count_threat(len(targets)),
            'behavior': behavior_score_raw,
            'proximity': proximity_score_raw,  # ADD THIS LINE
            'vital_signs': self.calculate_vital_signs_threat(targets, behavior_score_raw[0]),
        ...
        for name, (score, conf) in components_raw.items():
            adj_score = score  # Skip temporal escalation entirely for clean air
            components_adjusted[name] = adj_score
            confidences[name] = conf
            raw_scores[name] = score

        #if 'behavior' in components_adjusted:
        #    components_adjusted['proximity'] = components_adjusted['behavior']
        #    confidences['proximity'] = confidences['behavior']
        #    raw_scores['proximity'] = raw_scores['behavior']
        #else:
        #    components_adjusted['proximity'] = 0
        #    confidences['proximity'] = 0.5
        #    raw_scores['proximity'] = 0
        ...
        #if 'behavior' in adjusted_weights:
        #    adjusted_weights['proximity'] = adjusted_weights['behavior']
        #else:
        #    adjusted_weights['proximity'] = 0.15
        ...
        threat_entry = {
            ...
            'components': {
                name: {
                    ...
                    'raw_score': round(raw_scores.get(name, 0), 1),
                    'confidence': round(confidences[name], 2),
                    'weight': round(adjusted_weights.get(name, 0.15), 2)
                }
                for name in components_adjusted
            },
            'temporal': {
...
def compute_mq135_ppm(voltage):
    """Convert MQ135 voltage to approximate PPM using linear scaling"""
    if voltage <= 0 or math.isnan(voltage):
        return 0
    try:
        baseline_voltage = 0.15
        max_voltage = 0.5
        max_ppm = 50
        if voltage <= baseline_voltage:
            return 0
        ppm = ((voltage - baseline_voltage) / (max_voltage - baseline_voltage)) * max_ppm
        result = max(0, min(ppm, 1000))
        if result > 50:
            print(f"HIGH VOC: voltage={voltage:.4f}V ppm={result:.1f}")
        return result
    except:
        return 0
...
def analyze_odor(noise_db):
    ...
    global pm1_max, pm25_max, pm10_max
    ...
    try:
        ...
        if pms_data and len(pms_data) == 3:
            new_pm1, new_pm25, new_pm10 = pms_data
            pm1_max = max(pm1_max, new_pm1)
            pm25_max = max(pm25_max, new_pm25)
            pm10_max = max(pm10_max, new_pm10)
        pm1, pm25, pm10 = pm1_max, pm25_max, pm10_max
        
        voc_ppm_raw = compute_mq135_ppm(voc_voltage)
        if voc_ppm_raw > 50:
            print(f"VOC SPIKE: voltage={voc_voltage:.4f}V raw_ppm={voc_ppm_raw:.1f}")
        if VOC_BASELINE is None:
            voc_ppm = voc_ppm_raw
            VOC_BASELINE = voc_ppm_raw
        else:
            VOC_BASELINE = 0.70 * VOC_BASELINE + 0.30 * voc_ppm_raw
            voc_ppm = VOC_BASELINE
        ...
        print(f"ODOR RETURN: voc_ppm={round(voc_ppm,1)} voc_ppm_raw={round(voc_ppm_raw,1)} VOC_BASELINE={round(VOC_BASELINE,1) if VOC_BASELINE else None}")
        return {
            "timestamp": time.time(),
...
def init_hardware():
    ...
    try:
        ...
        mq135_channel = AnalogIn(ads, 1)  # A1 - Air quality
        sound_channel = AnalogIn(ads, 3)  # A3 - Microphone
        ...
        logger.info(f"  - Microphone: ADS1115 P1 (A3)")
        logger.info(f"  - Particle Sensor (PMS5003): Software UART")
...
try:
    pi_gpio.bb_serial_read_open(23, 9600, 8)
    logger.info("✅ PMS5003 pigpio bb_serial opened on GPIO 23")
except Exception as e:
    logger.error(f"Failed to open bb_serial: {e}")

# Initialize radar processor with software UART
radar_processor = RadarProcessor(use_software_uart=False)

# ==================== SENSOR READING FUNCTIONS ====================
def read_pms5003():
    """Read PMS5003 sensor via software UART"""
    if not pi_gpio:
        return None
    
    try:
        count, data = pi_gpio.bb_serial_read(23)
        if count > 0 and len(data) >= 32:
            # Find frame header
...
def read_sound():
    ...
    try:
        ...
        # Reject readings that look like MQ135 bleed (should be ~2.1V, not ~0.3V)
        if voltage < 1.0:
            return None
        return voltage
    except Exception:
...
def signal_handler(sig, frame):
    ...
    if sig == signal.SIGQUIT:  # Ctrl+\
        ...
        # Cleanup
        if pms and hasattr(pms, 'close'):
            pms.close()
        pi_gpio.bb_serial_read_close(23)
...
threat_scorer.temporal.threat_history.clear()
for key in threat_scorer.temporal.component_history:
    threat_scorer.temporal.component_history[key].clear()
...
def read_radar():
    """Wrapper function to read radar data from the radar processor"""
    if radar_processor:
        return radar_processor.read_radar()
    return None

def push_to_web(threat_data, quality_data, radar_data, sound_analysis, odor_analysis):
    """Push sensor data to Flask web app"""
    try:
        # Convert sound_analysis numpy types to native Python
        if sound_analysis:
            sound_clean = {
                'db': float(sound_analysis.get('db', 0)),
                'baseline': float(sound_analysis.get('baseline', 0)),
                'spike': bool(sound_analysis.get('spike', False)),
                'rate_of_change': float(sound_analysis.get('rate_of_change', 0)),
                'event': str(sound_analysis.get('event', 'unknown')),
                'confidence': float(sound_analysis.get('confidence', 0)),
            }
        else:
            sound_clean = None
        ...
        data = {
            ...
            'radar': radar_data,
            'sound': sound_clean,
            'sound_db': float(sound_analysis.get('db', 0)) if sound_analysis else 0,
            'sound_event': str(sound_analysis.get('event', 'unknown')) if sound_analysis else 'unknown',
            'sound_spike': bool(sound_analysis.get('spike', False)) if sound_analysis else False,
            'sound_baseline': float(sound_analysis.get('baseline', 0)) if sound_analysis else 0,
            'odor': {
                'air_quality_index': odor_analysis.get('air_quality_index', 0) if odor_analysis else 0,
                'voc_ppm': odor_analysis.get('voc_ppm', 0) if odor_analysis else 0,
                'pm25': odor_analysis.get('pm25', 0) if odor_analysis else 0,
                'odor_type': odor_analysis.get('odor_type', 'clean_air') if odor_analysis else 'clean_air',
                'odor_intensity': odor_analysis.get('odor_intensity', 0) if odor_analysis else 0,
                'classification_confidence': odor_analysis.get('classification_confidence', 0) if odor_analysis else 0,
            },
            'people_count': radar_data.get('target_count', 0) if radar_data else 0,
            'active_targets': sum(1 for t in radar_data.get('targets', []) if t.get('velocity', 0) > 0.1) if radar_data else 0,
            'abnormal_count': sum(1 for t in radar_data.get('targets', []) if t.get('abnormal_breathing', False)) if radar_data else 0,
            'avg_distance': round(
                sum(t.get('distance', 0) for t in radar_data.get('targets', [])) / max(len(radar_data.get('targets', [])), 1), 1
            ) if radar_data and radar_data.get('targets') else 0.0,
            'targets': radar_data.get('targets', []) if radar_data else [],
            'voc': odor_analysis.get('voc_ppm', 0) if odor_analysis else 0,
            'pm25': odor_analysis.get('pm25', 0) if odor_analysis else 0,
            'aqi': odor_analysis.get('air_quality_index', 0) if odor_analysis else 0,
            'odor_type': odor_analysis.get('odor_type', 'clean_air') if odor_analysis else 'clean_air',
            'odor_confidence': odor_analysis.get('classification_confidence', 0) if odor_analysis else 0,
            'odor_intensity': odor_analysis.get('odor_intensity', 0) if odor_analysis else 0,
            'last_update': datetime.now().strftime('%H:%M:%S'),
            'fake_mode': False,
            'sensor_status': {
                'radar': radar_data is not None,
                'pms5003': pms is not None,
                'mq135': mq135_channel is not None,
                'sound': sound_channel is not None
            }
        }
        json_str = json.dumps(data, default=lambda x: float(x) if hasattr(x, '__float__') else str(x))
        import json as _j; _parsed = _j.loads(json_str); print(f"PUSH VOC: {_parsed.get('voc')} odor_voc={_parsed.get('odor', {}).get('voc_ppm')}", flush=True)
...
def main():
    ...
    # Check hardware
    if None in [mq135_channel, sound_channel]:
    ...
    try:
        while True:
            ...
            try:
                ...
                radar_data = radar_processor.read_radar() if radar_processor else None

                print(f"\n🔍 RAW SENSOR VALUES:")
                print(f"   PMS5003: {pms_data}")
                print(f"   MQ135 voltage: {mq135_voltage:.4f}V")
                print(f"   Sound voltage: {sound_voltage:.4f}V" if sound_voltage is not None else "   Sound voltage: None (filtered)")
                print(f"   Radar data: {radar_data is not None}")
                if radar_data:
                    print(f"   Radar targets: {radar_data.get('target_count', 0)}")
                    print(f"   Radar targets list: {radar_data.get('targets', [])}")
                
                # Analyze sound
                sound_analysis = analyze_sound(sound_voltage) if sound_voltage is not None else None
                if sound_analysis:
                    last_sound_analysis = sound_analysis
                    logger.info(f"SOUND CACHED: {float(sound_analysis['db']):.1f} dB")
                else:
                    sound_analysis = last_sound_analysis
                    if sound_analysis:
                        logger.info(f"SOUND FROM CACHE: {float(sound_analysis['db']):.1f} dB")
                    else:
                        logger.info("SOUND: both None")

                # Analyze odor
                noise_db = sound_analysis['db'] if sound_analysis else 0
                odor_analysis = analyze_odor(noise_db)
                ...
                logger.info(f"DEBUG push sound: {sound_analysis}")
                push_to_web(threat_data, quality_data, radar_data, sound_analysis or last_sound_analysis, odor_analysis)
                ...
                if current_time - last_print_time >= print_interval:
                    ...
                    if pms_data:
                        ...
                        print(f"   PMS5003: No data (check /dev/serial0 connection)")
    
                    print(f"   MQ135: {mq135_voltage:.3f} V")

                    if sound_analysis:
                        print(f"   Sound: {sound_analysis['db']:.1f} dB | Event: {sound_analysis['event']} | Spike: {sound_analysis['spike']}")
                    else:
                        print(f"   Sound: {sound_voltage:.3f}V raw (analysis: None, samples: {len(samples)}/{WINDOW_SIZE})")

                    if radar_data and radar_data.get('targets'):
                        print(f"   Radar: {len(radar_data['targets'])} targets")
                        for t in radar_data['targets']:
                            print(f"      - {t.get('id')}: {t.get('distance', 0):.2f}m, {t.get('activity', 'unknown')}")
                    else:
                        print(f"   Radar: No targets")
