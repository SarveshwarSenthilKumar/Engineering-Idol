...
class LiveDataStore:
    ...
    def get_all_environments(self, fake_mode=False):
        ...
        with self.lock:
            ...
            for env_id, env_data in self.environments.items():
                ...
                env_copy['online'] = env_data.get('last_update') is not None
...
live_data.latest = {}
for env in live_data.environments:
    live_data.environments[env]['data'] = {}
    live_data.environments[env]['threat_score'] = 0
    ...
    try:
        ...
        cursor.execute("""
            SELECT 
                ...
                0 as avg_behavior,
                0 as avg_vital_signs,
                AVG(air_aqi) as avg_air_quality,
                AVG(sound_db) as avg_noise
            FROM events_log
            WHERE timestamp >= ?
        """, (cutoff,))
    ...
    try:
        ...
        if latest_event:
            ...
            real_data = {
                ...
                'active_targets': event_data.get('active_targets',
                    sum(1 for t in event_data.get('targets', []) if t.get('velocity', 0) > 0.1)),
                'abnormal_count': event_data.get('abnormal_count',
                    sum(1 for t in event_data.get('targets', []) if t.get('abnormal_breathing', False))),
                'avg_distance': event_data.get('avg_distance',
                    round(sum(t.get('distance', 0) for t in event_data.get('targets', [])) /
                          max(len(event_data.get('targets', [])), 1), 1)
                    if event_data.get('targets') else 0.0),
...
def dashboard():
    ...
    fake_mode = session.get('fake_mode', False)
...
def api_update():
    ...
    if request.method == 'POST':
        data = request.json
        # Filter out bad VOC spikes — keep previous good value instead
        voc = data.get('voc', 0)
        if voc > 60:
            prev = live_data.get_latest()
            prev_voc = prev.get('voc', 0) if prev else 0
            if prev_voc > 0 and prev_voc <= 60:
                # Replace spike with last known good value
                data['voc'] = prev_voc
                if data.get('odor') and prev.get('odor'):
                    data['odor']['voc_ppm'] = prev.get('odor', {}).get('voc_ppm', prev_voc)
                    data['odor']['air_quality_index'] = prev.get('odor', {}).get('air_quality_index', 0)
                data['aqi'] = prev.get('aqi', 0)
            else:
                # No good previous value — skip this update entirely
...
def api_live():
    ...
    if fake_mode:
        ...
        if data and data.get('last_update'):
            try:
                last_update = datetime.strptime(data['last_update'], '%H:%M:%S').replace(
